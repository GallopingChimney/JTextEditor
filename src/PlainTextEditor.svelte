<script>
  ...
  const LANG_MAP = {
    ...
    css: 'css', json: 'json', py: 'python', rs: 'rust',
    md: 'markdown', sh: 'bash', bash: 'bash', yml: 'yaml', yaml: 'yaml',
    c: 'c', cpp: 'cpp', h: 'c', hpp: 'cpp',
    cs: 'csharp', java: 'java', go: 'go', rb: 'ruby',
    php: 'php', swift: 'swift', kt: 'kotlin', lua: 'lua',
    pl: 'perl', r: 'r', scala: 'scala', dart: 'dart',
    ps1: 'powershell', toml: 'toml', sql: 'sql',
    dockerfile: 'docker', ini: 'ini', cfg: 'ini', diff: 'diff',
  };
  ...
  function addMatchHighlights(html, term, activeCharIdx) {
    ...
    for (const m of matches) {
      ...
      const cls = m.idx === activeCharIdx ? 'jte-match jte-match-active' : 'jte-match';
  ...
  function handleCursor() {
    ...
    oncursor?.({ line: lines.length, col: lines[lines.length - 1].length + 1 });
  }
  ...
  function buildFindRegex(term) {
    if (!term) return null;
    let pattern = findOpts.useRegex ? term : term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  ...
  function doFindNext(term) {
    ...
    searchTerm = term;
    const matches = findAllMatches(term);
    if (!matches.length) { currentMatchIdx = -1; return; }
  ...
  function doFindPrev(term) {
    ...
    for (let i = matches.length - 1; i >= 0; i--) {
      if (matches[i].idx < before) { match = matches[i]; break; }
    }
  ...
  function doReplace(search, replace) {
    ...
    const replacement = findOpts.useRegex
      ? content.slice(match.idx, match.idx + match.len).replace(buildFindRegex(search), replace)
      : replace;
    textarea.value = before + replacement + after;
    content = textarea.value;
    onchange?.(content);
    textarea.selectionStart = textarea.selectionEnd = match.idx + replacement.length;
    doFindNext(search);
  }
  ...
  $effect(() => {
    ...
    const { type, search, replace, _tick, matchCase, wholeWord, useRegex } = findAction;
    if (_tick === lastFindTick) return;
    lastFindTick = _tick;
    findOpts = { matchCase: !!matchCase, wholeWord: !!wholeWord, useRegex: !!useRegex };
    switch (type) {
      case 'next': doFindNext(search); break;
      case 'prev': doFindPrev(search); break;
      case 'replace': doReplace(search, replace); break;
      case 'replaceAll': doReplaceAll(search, replace); break;
      case 'clear': searchTerm = ''; currentMatchIdx = -1; break;
    }
  });

  $effect(() => { onmatchchange?.(currentMatchIdx); });
  ...
  let highlightedHtml = $derived.by(() => {
    let html = language ? syntaxHighlight(content, language) : escapeHtml(content);
    if (searchTerm) {
    ...
    if (highlightLine && currentLine > 0) {
      ...
      if (idx < lines.length) {
        ...
        lines[idx] = `<span class="jte-current-line">${lineContent}</span>`;
      }
      html = lines.join('\n');
    }
    return html;
  ...
  {#if showLineNumbers}
    ...
    >
      {#each Array(lineCount) as _, i}
        <div class="jte-line-num" class:active={highlightLine && i + 1 === currentLine}>{i + 1}</div>
      {/each}
    </div>
  {/if}
...
<style>
  ...
  .jte-overlay :global(.token.cdata) { color: #6a9955; }
  .jte-overlay :global(.token.punctuation) { color: #d4d4d4; }
  ...
  .jte-overlay :global(.token.symbol) { color: #b5cea8; }
